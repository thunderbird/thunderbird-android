...
public class Utility {
    ...
    private static final Pattern VALID_DOMAIN_NAME_1 = Pattern.compile("^([a-zA-Z0-9]([a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?\\.)*[a-zA-Z0-9]([a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?$");
    private static final Pattern VALID_DOMAIN_NAME_2 = Pattern.compile("^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$");
    
    public static boolean domainFieldValid(EditText view) {
        if (view.getText() != null) {
            String s = view.getText().toString();
            if (s.length() <= 253 && VALID_DOMAIN_NAME_1.matcher(s).matches()) {
                return true;
            }
            if (VALID_DOMAIN_NAME_2.matcher(s).matches()) {
                return true;
            }
        }
        return false;

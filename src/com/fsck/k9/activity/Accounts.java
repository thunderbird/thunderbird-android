		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (stats != null) {
				...
				holder.flaggedMessageCountWrapper
						.setVisibility(stats.flaggedMessageCount > 0 ? View.VISIBLE
								: View.GONE);

				holder.flaggedMessageCountWrapper
						.setOnClickListener(createFlaggedSearch(account));
				holder.newMessageCountWrapper
						.setOnClickListener(createUnreadSearch(account));
			...
			if (account instanceof Account) {
				...
				if (unreadMessageCount == null) {
					...
					holder.chip
							.setBackgroundDrawable(realAccount
									.generateColorChip(true, false, false,
											false, false).drawable());
				} else {
					holder.chip.setBackgroundDrawable(realAccount
							.generateColorChip(false, false, false, false,
									false).drawable());
				}
				...
				holder.chip.setBackgroundDrawable(new ColorChip(0xff999999,
						false, ColorChip.CIRCULAR).drawable());
			...
			if (account instanceof SearchAccount) {
				...
				holder.syncCheckBox.setVisibility(View.GONE);
			} else {
				holder.folders.setVisibility(View.VISIBLE);
				holder.syncCheckBox.setVisibility(View.VISIBLE);
				// if the account notifies new mail
				if (((Account) account).isNotifyNewMail()) {
					Log.v("Account CheckBox Activation", "Account enabled");
					// Check the box
					holder.syncCheckBox.setChecked(true);
				} else {
					// else uncheck the box
					Log.v("Account CheckBox Activation", "Account disabled");
					holder.syncCheckBox.setChecked(false);
				}
							...
							public void onCheckedChanged(
								...
								if (((Account) account).isNotifyNewMail()) {
									Toast.makeText(
											getApplication(),
											getString(
													R.string.account_notify_on_toast,
													account),
									...
									Toast.makeText(
											getApplication(),
											getString(
													R.string.account_notify_off_toast,
													account),
											Toast.LENGTH_SHORT).show();
								}
							}

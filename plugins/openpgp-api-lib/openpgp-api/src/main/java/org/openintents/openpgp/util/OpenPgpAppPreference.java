...
public class OpenPgpAppPreference extends DialogPreference {
    ...
    private static final Intent MARKET_INTENT = new Intent(Intent.ACTION_VIEW, Uri.parse(
            String.format(MARKET_INTENT_URI_BASE, OPEN_KEYCHAIN_PACKAGE)));
    ...
    private ArrayList<OpenPgpProviderEntry> legacyList = new ArrayList<>();
    private ArrayList<OpenPgpProviderEntry> list = new ArrayList<>();

    private String selectedPackage;

    private OpenPgpAppPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        populateAppList();
    }
    ...
    public void addLegacyProvider(int position, String packageName, String simpleName, Drawable icon) {
        legacyList.add(position, new OpenPgpProviderEntry(packageName, simpleName, icon));
    }
    ...
    public void addLegacyProvider(String packageName, String simpleName, int iconRes) {
        ...
        legacyList.add(new OpenPgpProviderEntry(packageName, simpleName, icon));
    }
    ...
    protected void onPrepareDialogBuilder(Builder builder) {
        ...
        ListAdapter adapter = new ArrayAdapter<OpenPgpProviderEntry>(getContext(),
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                tv.setCompoundDrawablesWithIntrinsicBounds(list.get(position).icon, null,
                        null, null);
        ...
        builder.setSingleChoiceItems(adapter, getIndexOfProviderList(selectedPackage),
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        OpenPgpProviderEntry entry = list.get(which);
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (positiveResult && (selectedPackage != null)) {
            save();
        }
    }
    ...
    private void save() {
        ...
        // invalid
        if (!callChangeListener(selectedPackage)) {
            // They don't want the value to be set
            return;
        }

        setAndPersist(selectedPackage);
    }
    ...
    private int getIndexOfProviderList(String packageName) {
        for (OpenPgpProviderEntry app : list) {
    ...
    private void populateAppList() {
        ...
        // add "none"-entry
        list.add(0, new OpenPgpProviderEntry("",
        ...
        if (resInfo != null) {
            for (ResolveInfo resolveInfo : resInfo) {
                ...
                if (!PROVIDER_BLACKLIST.contains(packageName)) {
                    list.add(new OpenPgpProviderEntry(packageName, simpleName, icon));
        ...
        if (!hasNonBlacklistedChoices) {
            ...
            for (ResolveInfo resolveInfo : resInfo) {
                ...
                list.add(new OpenPgpProviderEntry(OPEN_KEYCHAIN_PACKAGE, simpleName,
                        icon, marketIntent));
            }
        }
    }

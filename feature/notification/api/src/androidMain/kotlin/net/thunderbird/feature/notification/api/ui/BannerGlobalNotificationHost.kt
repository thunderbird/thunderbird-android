...
fun BannerGlobalNotificationHost(
    ...
    val bannerGlobal = hostStateHolder.currentInAppNotificationHostState.bannerGlobalVisual
    AnimatedContent(
        targetState = bannerGlobal,
        modifier = modifier.testTagAsResourceId(BannerGlobalNotificationHostDefaults.HOST_TEST_TAG),
...
private fun BannerGlobalNotificationHostLayout(
    ...
    when (visual.severity) {
        NotificationSeverity.Fatal, NotificationSeverity.Critical -> ErrorBannerGlobalNotificationCard(
            text = visual.message,
            action = action,
            modifier = modifier.testTagAsResourceId(BannerGlobalNotificationHostDefaults.ERROR_BANNER_TEST_TAG),
        )

        NotificationSeverity.Warning -> WarningBannerGlobalNotificationCard(
            text = visual.message,
            action = action,
            modifier = modifier.testTagAsResourceId(BannerGlobalNotificationHostDefaults.WARNING_BANNER_TEST_TAG),
        )

        NotificationSeverity.Temporary, NotificationSeverity.Information -> InfoBannerGlobalNotificationCard(
            text = visual.message,
            action = action,
            modifier = modifier.testTagAsResourceId(BannerGlobalNotificationHostDefaults.INFO_BANNER_TEST_TAG),
        )
    }
}

internal object BannerGlobalNotificationHostDefaults {
    const val HOST_TEST_TAG = "banner_global_notification_host"
    const val ERROR_BANNER_TEST_TAG = "error_banner_global_notification"
    const val WARNING_BANNER_TEST_TAG = "warning_banner_global_notification"
    const val INFO_BANNER_TEST_TAG = "info_banner_global_notification"
}

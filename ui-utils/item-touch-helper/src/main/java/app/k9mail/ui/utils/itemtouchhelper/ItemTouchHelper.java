...
/**
 * Fork of {@link androidx.recyclerview.widget.ItemTouchHelper} that supports horizontal swipe actions that don't remove
 * the list item. In that case item views are not animated all the way off the screen.
 * <br>
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    /**
     * Horizontal start direction. Resolved to LEFT or RIGHT depending on RecyclerView's layout direction. Used for
     * swipe & drag control.
     */
    public static final int START = LEFT << 2;

    /**
     * Horizontal end direction. Resolved to LEFT or RIGHT depending on RecyclerView's layout direction. Used for swipe
     * & drag control.
     */
    ...
    /**
     * Views, whose state should be cleared after they are detached from RecyclerView. This is necessary after swipe
     * dismissing an item. We wait until animator finishes its job to clean these views.
     */
    ...
    /**
     * The reference coordinates for the action start. For drag & drop, this is the time long press is completed vs for
     * swipe, this is the initial touch point.
     */
    ...
    /**
     * The coordinates of the selected view at the time it is selected. We record these values when action starts so
     * that we can consistently position it even if LayoutManager moves the View.
     */
    ...
    /**
     * The direction flags obtained from unmasking {@link Callback#getAbsoluteMovementFlags(RecyclerView, ViewHolder)}
     * for the current action state.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
        int mSelectedFlags;

    /**
     * When a View is dragged or swiped and needs to go back to where it was, we create a Recover Animation and animate
     * it to its location using this custom Animator, instead of using framework Animators. Using framework animators
     * has the side effect of clashing with ItemAnimator, creating jumpy UIs.
     */
    @VisibleForTesting
    ...
    /**
     * When user drags a view to the edge, we start scrolling the LayoutManager as long as View is partially out of
     * bounds.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    ...
    /**
     * This keeps a reference to the child dragged by the user. Even after user stops dragging, until view reaches its
     * final position (end of recover animation), we keep a reference so that it can be drawn above other children.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
        View mOverdrawChild = null;

    /**
     * We cache the position of the overdraw child to avoid recalculating it each time child position callback is
     * called. This value is invalidated whenever a child is attached or detached.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
     ...
     * <p>
     * You can attach ItemTouchHelper to a RecyclerView via {@link #attachToRecyclerView(RecyclerView)}. Upon attaching,
     * it will add an item decoration, an onItemTouchListener and a Child attach / detach listener to the RecyclerView.
     *
    ...
    /**
     * Attaches the ItemTouchHelper to the provided RecyclerView. If TouchHelper is already attached to a RecyclerView,
     * it will first detach from the previous one. You can call this method with {@code null} to detach it from the
     * current RecyclerView.
     *
     * @param recyclerView The RecyclerView instance to which you want to add this helper or {@code null} if you want to
     *                     remove ItemTouchHelper from the current RecyclerView.
     */
     ...
     *
     * @param selected    The ViewHolder to drag or swipe. Can be null if you want to cancel the current action, but may
     *                    not be null if actionState is ACTION_STATE_DRAG.
    ...
    boolean hasRunningRecoverAnim() {
        for (RecoverAnimation mRecoverAnimation : mRecoverAnimations) {
    ...
    void moveIfNecessary(ViewHolder viewHolder) {
        ...
        final float threshold = mCallback.getMoveThreshold();
    ...
    private int swipeIfNecessary(ViewHolder viewHolder) {
        ...
        if (Math.abs(mDx) > Math.abs(mDy)) {
            if ((swipeFlags = checkHorizontalSwipe(viewHolder, flags)) > 0) {
                ...
                if ((originalFlags & swipeDir) == 0) {
                    ...
                    return Callback.convertToRelativeDirection(swipeDir,
                        mRecyclerView.getLayoutDirection()) | fling;
                }
                return swipeFlags;
            }
            ...
            if ((swipeFlags = checkHorizontalSwipe(viewHolder, flags)) > 0) {
                ...
                if ((originalFlags & swipeDir) == 0) {
                    // convert to relative
                    return Callback.convertToRelativeDirection(swipeDir, mRecyclerView.getLayoutDirection()) | fling;
                }
                return swipeFlags;
            }
        }
    ...
    /**
     * An interface which can be implemented by LayoutManager for better integration with {@link ItemTouchHelper}.
     */
    public interface ViewDropHandler {
         ...
         * <p>
         * For example, LinearLayoutManager sets up a "scrollToPositionWithOffset" calls so that the View under drag
         * will be used as an anchor View while calculating the next layout, making layout stay consistent.
         *
         * @param view   The View which is being dragged. It is very likely that user is still dragging this View so
         *               there might be other calls to {@code prepareForDrop()} after this one.
         * @param target The target view which is being dropped on.
         * @param x      The <code>left</code> offset of the View that is being dragged. This value includes the
         *               movement caused by the user.
         * @param y      The <code>top</code> offset of the View that is being dragged. This value includes the movement
         *               caused by the user.
         */
     ...
     * {@link #getMovementFlags(RecyclerView, ViewHolder)} and return appropriate set of direction flags.
     * ({@link #LEFT}, {@link #RIGHT}, {@link #START}, {@link #END}, {@link #UP}, {@link #DOWN}). You can use
     * {@link #makeMovementFlags(int, int)} to easily construct it. Alternatively, you can use {@link SimpleCallback}.
     * <p>
     * If user drags an item, ItemTouchHelper will call
     * {@link Callback#onMove(RecyclerView, ViewHolder, ViewHolder) onMove(recyclerView, dragged, target)}. Upon
     * receiving this callback, you should move the item from the old position ({@code dragged.getAdapterPosition()}) to
     * new position ({@code target.getAdapterPosition()}) in your adapter and also call
     * {@link RecyclerView.Adapter#notifyItemMoved(int, int)}. When a dragging View overlaps multiple other views,
     * Callback chooses the closest View with which dragged View might have changed positions. Although this approach
     * works for many use cases, if you have a custom LayoutManager, you can override
     * {@link #chooseDropTarget(ViewHolder, java.util.List, int, int)} to select a custom drop target.
     * <p>
     * When a View is swiped, ItemTouchHelper animates it until it goes out of bounds, then calls
     * {@link #onSwiped(ViewHolder, int)}. At this point, you should update your adapter (e.g. remove the item) and call
    ...
    public abstract static class Callback {
         ...
         * @param layoutDirection The layout direction of the View.
         * @return Updated flags which uses relative flags ({@link #START}, {@link #END}) instead of {@link #LEFT},
         * {@link #RIGHT}.
         ...
         * <p>
         * For instance, if you want to let your items be drag & dropped vertically and swiped left to be dismissed, you
         * can call this method with:
         ...
         *
         * @param actionState The action state you want to get flags in. Should be one of {@link #ACTION_STATE_IDLE},
         *                    {@link #ACTION_STATE_SWIPE} or {@link #ACTION_STATE_DRAG}.
         * @param directions  The direction flags. Can be composed from {@link #UP}, {@link #DOWN}, {@link #RIGHT},
         *                    {@link #LEFT} {@link #START} and {@link #END}.
        ...
        /**
         * Should return a composite flag which defines the enabled move directions in each state (idle, swiping,
         * dragging).
         * <p>
         * Instead of composing this flag manually, you can use {@link #makeMovementFlags(int, int)} or
         * {@link #makeFlag(int, int)}.
         * <p>
         * This flag is composed of 3 sets of 8 bits, where first 8 bits are for IDLE state, next 8 bits are for SWIPE
         * state and third 8 bits are for DRAG state. Each 8 bit sections can be constructed by simply OR'ing direction
         * flags defined in {@link ItemTouchHelper}.
         * <p>
        ...
        /**
         * Converts a given set of flags to absolution direction which means {@link #START} and {@link #END} are
         * replaced with {@link #LEFT} and {@link #RIGHT} depending on the layout direction.
         *
         * @param flags           The flag value that include any number of movement flags.
         ...
         * <p>
         * This method is used when selecting drop target for the dragged View. After Views are eliminated either via
         * bounds check or via this method, resulting set of views will be passed to
         * {@link #chooseDropTarget(ViewHolder, java.util.List, int, int)}.
         * <p>
         ...
         * <p>
         * If this method returns true, ItemTouchHelper assumes {@code viewHolder} has been moved to the adapter
         * position of {@code target} ViewHolder
         * ({@link ViewHolder#getAbsoluteAdapterPosition() ViewHolder#getAdapterPositionInRecyclerView()}).
         * <p>
         ...
         * @see #onMoved(RecyclerView, ViewHolder, ViewHolder, int, int, int)
         */
         ...
         * <p>
         * Default value returns true
         *
         * @return True if ItemTouchHelper should start dragging an item when it is long pressed, false otherwise.
         ...
         * Default value returns true
         *
         * @return True if ItemTouchHelper should start swiping an item when user swipes a pointer over the View, false
        ...
        /**
         * Returns the fraction that the user should move the View to be considered as swiped. The fraction is
         * calculated with respect to RecyclerView's bounds.
         * <p>
         * Default value is .5f, which means, to swipe a View, user must move the View at least half of RecyclerView's
         * width or height, depending on the swipe direction.
         *
        ...
        /**
         * Returns the fraction that the user should move the View to be considered as it is dragged. After a view is
         * moved this amount, ItemTouchHelper starts checking for Views below it for a possible drop.
         *
         * @return A float value that denotes the fraction of the View size. Default value is .5f .
         */
        @SuppressWarnings("WeakerAccess")
         ...
         * <p>
         * You can increase this value to make it harder to swipe or decrease it to make it easier. Keep in mind that
         * ItemTouchHelper also checks the perpendicular velocity and makes sure current direction velocity is larger
         * then the perpendicular one. Otherwise, user's movement is ambiguous. You can change the threshold by
         * overriding {@link #getSwipeVelocityThreshold(float)}.
         * <p>
         ...
         * <p>
         * To consider a movement as swipe, ItemTouchHelper requires it to be larger than the perpendicular movement. If
         * both directions reach to the max threshold, none of them will be considered as a swipe because it is usually
         * an indication that user rather tried to scroll then swipe.
         * <p>
         * The velocity is calculated in pixels per second.
         * <p>
         * You can customize this behavior by changing this method. If you increase the value, it will be easier for the
         * user to swipe diagonally and if you decrease the value, user will need to make a rather straight finger
         * movement to trigger a swipe.
         *
         * @param defaultValue The default value(in pixels per second) used by the ItemTouchHelper.
        ...
        /**
         * Called by ItemTouchHelper to select a drop target from the list of ViewHolders that are under the dragged
         * View.
         * <p>
         * Default implementation filters the View with which dragged item have changed position in the drag direction.
         * For instance, if the view is dragged UP, it compares the
         ...
         * <p>
         * This method is called on the main thread every time user moves the View. If you want to override it, make
         * sure it does not do any expensive operations.
         *
         ...
         * @param curX        The updated left value of the dragged View after drag translations are applied. This value
         *                    does not include margins added by {@link RecyclerView.ItemDecoration}s.
         * @param curY        The updated top value of the dragged View after drag translations are applied. This value
         *                    does not include margins added by {@link RecyclerView.ItemDecoration}s.
        ...
        public ViewHolder chooseDropTarget(@NonNull ViewHolder selected,
            ...
            for (final ViewHolder target : dropTargets) {
                if (dx > 0) {
         ...
         * {@link #getMovementFlags(RecyclerView, ViewHolder)} method, this method will also use relative directions.
         * Otherwise, it will use absolute directions.
         * <p>
         * If you don't support swiping, this method will never be called.
         * <p>
         * ItemTouchHelper will keep a reference to the View until it is detached from RecyclerView. As soon as it is
         * detached, ItemTouchHelper will call {@link #clearView(RecyclerView, ViewHolder)}.
         *
         * @param viewHolder The ViewHolder which has been swiped by the user.
         * @param direction  The direction to which the ViewHolder is swiped. It is one of {@link #UP}, {@link #DOWN},
         *                   {@link #LEFT} or {@link #RIGHT}. If your
         *                   {@link #getMovementFlags(RecyclerView, ViewHolder)} method returned relative flags instead
         *                   of {@link #LEFT} / {@link #RIGHT}; `direction` will be relative as well. ({@link #START} or
         *                   {@link #END}).
         */
         ...
         */
        public void onSelectedChanged(@Nullable ViewHolder viewHolder) {
            if (viewHolder != null) {
         ...
         * <p>
         * ItemTouchHelper does not create an extra Bitmap or View while dragging, instead, it modifies the existing
         * View. Because of this reason, it is important that the View is still part of the layout after it is moved.
         * This may not work as intended when swapped Views are close to RecyclerView bounds or there are gaps between
         * them (e.g. other Views which were not eligible for dropping over).
         * <p>
         * This method is responsible to give necessary hint to the LayoutManager so that it will keep the View in
         * visible area.
         * <p>
         * Default implementation calls {@link RecyclerView#scrollToPosition(int)} if the View's new position is likely
         * to be out of bounds.
         * <p>
         * It is important to ensure the ViewHolder will stay visible as otherwise, it might be removed by the
         * LayoutManager if the move causes the View to go out of bounds. In that case, drag will end prematurely.
         *
         * @param recyclerView The RecyclerView controlled by the ItemTouchHelper.
         * @param viewHolder   The ViewHolder under user's control.
         * @param target       The ViewHolder on which the currently active item has been dropped.
         * @param toPos        The new adapter position of the dragged item.
         * @param x            The updated left value of the dragged View after drag translations are applied. This
         *                     value does not include margins added by {@link RecyclerView.ItemDecoration}s.
         * @param y            The updated top value of the dragged View after drag translations are applied. This value
         *                     does not include margins added by {@link RecyclerView.ItemDecoration}s.
         */
        public void onMoved(@NonNull final RecyclerView recyclerView,
            @NonNull final ViewHolder viewHolder, @NonNull final ViewHolder target,
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
            ...
            for (final RecoverAnimation anim : recoverAnimationList) {
                anim.update();
         ...
         * {@link #onSelectedChanged(RecyclerView.ViewHolder)},
         * {@link #onChildDraw(Canvas, RecyclerView, ViewHolder, float, float, int, boolean, boolean)} or
         ...
         * <p>
         * If you would like to customize how your View's respond to user interactions, this is a good place to
         * override.
         * <p>
         * Default implementation translates the child by the given <code>dX</code>,
         * <code>dY</code>.
         * ItemTouchHelper also takes care of drawing the child after other children if it is being dragged. This is
         * done using child re-ordering mechanism. On platforms prior to L, this is achieved via
         * {@link android.view.ViewGroup#getChildDrawingOrder(int)} and on L and after, it changes View's elevation
         * value to be greater than all other children.)
         *
         ...
         * @param actionState       The type of interaction on the View. Is either {@link #ACTION_STATE_DRAG} or
         *                          {@link #ACTION_STATE_SWIPE}.
         * @param isCurrentlyActive True if this view is currently being controlled by the user or false if it is simply
         ...
         * {@link RecyclerView.ItemAnimator#getRemoveDuration()}. If RecyclerView does not have any
         * {@link RecyclerView.ItemAnimator} attached, this method returns {@code DEFAULT_DRAG_ANIMATION_DURATION} or
         * {@code DEFAULT_SWIPE_ANIMATION_DURATION} depending on the animation type.
         *
         ...
         * Default implementation calculates a value based on the amount of View out of bounds and the time it spent
         * there. The longer user keeps the View out of bounds, the faster the list will scroll. Similarly, the larger
         ...
         * @param viewSizeOutOfBounds The total size of the View that is out of bounds. This value is negative if the
         *                            View is dragged towards left or top edge.
         * @param msSinceStartScroll  The time passed since View is kept out of bounds.
         * @return The amount that RecyclerView should scroll. Keep in mind that this value will be passed to
         * {@link RecyclerView#scrollBy(int, int)} method.
         */
        @SuppressWarnings("WeakerAccess")
    ...
    /**
     * A simple wrapper to the default Callback which you can construct with drag and swipe directions and this class
     * will handle the flag callbacks. You should still override onMove or onSwiped depending on your use case.
     *
     * <pre>
    ...
    public abstract static class SimpleCallback extends Callback {
        ...
        /**
         * Creates a Callback for the given drag and swipe allowance. These values serve as defaults and if you want to
         * customize behavior per ViewHolder, you can override {@link #getSwipeDirs(RecyclerView, ViewHolder)} and / or
         * {@link #getDragDirs(RecyclerView, ViewHolder)}.
         *
         * @param dragDirs  Binary OR of direction flags in which the Views can be dragged. Must be composed of
         *                  {@link #LEFT}, {@link #RIGHT}, {@link #START}, {@link #END}, {@link #UP} and {@link #DOWN}.
        ...
        /**
         * Updates the default swipe directions. For example, you can use this method to toggle certain directions
        ...
        /**
         * Updates the default drag directions. For example, you can use this method to toggle certain directions
         * depending on your use case.
         *
        ...
        /**
         * Returns the swipe directions for the provided ViewHolder. Default implementation returns the swipe directions
         * that was set via constructor or {@link #setDefaultSwipeDirs(int)}.
         *
         * @param recyclerView The RecyclerView to which the ItemTouchHelper is attached to.
        ...
        /**
         * Returns the drag directions for the provided ViewHolder. Default implementation returns the drag directions
         * that was set via constructor or {@link #setDefaultDragDirs(int)}.
         *
         * @param recyclerView The RecyclerView to which the ItemTouchHelper is attached to.
         * @param viewHolder   The ViewHolder for which the swipe direction is queried.
         * @return A binary OR of direction flags.
    ...
    private class ItemTouchHelperGestureListener extends GestureDetector.SimpleOnGestureListener {
         ...
         * <p>
         * It is necessary to control this here because {@link GestureDetector.SimpleOnGestureListener} can only be set
         * on a {@link GestureDetector} in a GestureDetector's constructor, a GestureDetector will call onLongPress if
         * an {@link MotionEvent#ACTION_DOWN} event is not followed by another event that would cancel it (like
         * {@link MotionEvent#ACTION_UP} or {@link MotionEvent#ACTION_CANCEL}), the long press responding to the long
         * press event needs to be cancellable to prevent unexpected behavior.
         *
    ...
    static class RecoverAnimation implements Animator.AnimatorListener {
        ...
        /**
         * We run updates on onDraw method but use the fraction from animator callback. This way, we can sync translate
         * x/y values w/ the animators to avoid one-off frames.
         */
        public void update() {

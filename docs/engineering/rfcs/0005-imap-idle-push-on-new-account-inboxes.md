...
`isPushEnabled = true` when creating a folder details instance for the account's inbox folder.
...
deal of dependency changes to make the information available. This will require further technical investigation.
...
frequently and reduce both battery usage and network bandwidth. As a result, it may reduce battery consumption for our
users. Those who see an increase would likely have an account where they receive many emails, but not all at once, such
as automated emails every few minutes. Someone with such a setup could turn off the setting where it exists today. In a
future project, we'll make these settings even easier to access.

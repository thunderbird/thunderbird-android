...
class Address private constructor(
    ...
    init {
        ...
        if (parse) {
            ...
            if (tokens.size > 0) {
                val token = tokens[0]
                requireNotNull(token.address) { "token.getAddress()" }
                ...
                if (!isEmpty(name)) {
                    ...
                    this.personal = personal?.trim { it <= ' ' }
                }
            } else {
    ...
    constructor(address: Address) : this(address.mAddress!!, address.personal, false)

    constructor(address: String, personal: String?) : this(address, personal, true)

    constructor(address: String) : this(address, null, true)
    ...
    val hostname: String
        get() {
            ...
            val hostIdx = mAddress!!.lastIndexOf("@")
            if (hostIdx == -1) {
                return ""
            }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (mAddress != address.mAddress) {
            return false
        }
        return personal == address.personal
    }
    ...
    override fun toString(): String =
        if (isEmpty(this.personal)) {
            mAddress!!
        } else {
            quoteAtoms(this.personal!!) + " <" + mAddress + ">"
        }
    ...
    fun toEncodedString(): String =
        if (isEmpty(this.personal)) {
            ...
            EncoderUtil.encodeAddressDisplayName(this.personal) + " <" + mAddress + ">"
        }
    ...
    fun needsUnicode(): Boolean {
        if (mAddress == null) return false
        var i = mAddress!!.length - 1
        while (i >= 0 && mAddress!![i].code < ASCII_LIMIT) i--
        return i >= 0
    }
    ...
    companion object {
        ...
        fun parseUnencoded(addressList: String?): Array<Address> {
            ...
            return tokens
                .filter { !isEmpty(it.address) }
                .map { Address(it.address!!, it.name, false) }
                .toTypedArray()
        }
        ...
        fun parse(addressList: String?): Array<Address> {
            ...
            return try {
                ...
                Array(parsedList.size) { i ->
                    val m = parsedList[i]
                    Address("${m.localPart}@${m.domain}", m.name, false)
                }
            } catch (pe: MimeException) {
                e(pe, "MimeException in Address.parse()")

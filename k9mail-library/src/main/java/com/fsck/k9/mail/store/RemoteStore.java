...
public abstract class RemoteStore extends Store {
    ...
    public synchronized static Store getInstance(Context context, StoreConfig storeConfig, ProxySettings proxySettings)
        ...
        if (store == null) {
            if (uri.startsWith("imap")) {
                ...
                store = new ImapStore(
                        ...
                        oAuth2TokenProvider, proxySettings);
            } else if (uri.startsWith("pop3")) {
                store = new Pop3Store(storeConfig, new DefaultTrustedSocketFactory(context, proxySettings));

...
class StateMachineBuilder<TState : Any, TEvent : Any> internal constructor() {
    ...
    private val stateRegistrar = mutableMapOf<KClass<out TState>, StateRegistry<TState, TEvent>>()
     ...
     */
    fun finalState(
        finalStateClass: KClass<out TState>,
        init: StateWithoutTransactionsBuilder<TState, TEvent>.() -> Unit = {},
    ) {
    ...
    inline fun <reified TFinalState : TState> finalState(
        noinline init: StateWithoutTransactionsBuilder<TState, TEvent>.() -> Unit = {},
    ) {
    ...
    fun <TCurrentState : TState> state(
        ...
        val stateRegistry = StateBuilderImpl<TCurrentState, TState, TEvent>(stateClass).apply(init).build()
        stateRegistrar += stateClass to stateRegistry
    }

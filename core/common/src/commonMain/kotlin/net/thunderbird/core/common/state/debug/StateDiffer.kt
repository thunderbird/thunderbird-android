...
internal class StateDiffer(
     ...
     */
    fun buildDiff(old: Any?, new: Any?, context: DiffContext): String = buildString {
        if (old == null && new == null) return@buildString

        val oldProps = old?.toPropertyMap().orEmpty()
        val newProps = new?.toPropertyMap().orEmpty()

        if (oldProps.isEmpty() && newProps.isEmpty()) {
            appendScalarDiff(oldVal = old, newVal = new, context = context)
        } else {
            appendDiffedObjectProperties(oldProps, newProps, context)
        }
    }

    private fun StringBuilder.appendDiffedObjectProperties(
    ...
    private fun StringBuilder.appendPropertyDiff(
        ...
        when {
            ...
            else -> appendScalarDiff(key = key, oldVal = oldVal, newVal = newVal, context = context)
        }
    }

    private fun StringBuilder.appendScalarDiff(oldVal: Any?, newVal: Any?, context: DiffContext, key: String? = null) {
    ...
    private fun StringBuilder.appendMapDiff(key: String, oldVal: Map<*, *>?, newVal: Map<*, *>?, context: DiffContext) {
        ...
        val allMapKeys = oldMap.keys + newMap.keys
        ...
        when {
            ...
            hasChanges -> appendEntryChanges(
                ...
                formatValue = { valueFormatter.format(it?.value) },
                openingMarker = "{",
                closingMarker = "}",
                context = context,
            )

            context.isStateClassChanged ->
                appendLine(
                    StatePrettyPrinterVocabulary.buildMapSameString(
                        key = key,
                        size = oldSize,
                        indentSize = context.indentSize,
                    ),
    ...
    private fun StringBuilder.appendListDiff(
        ...
        when {
            ...
            context.isStateClassChanged ->
                appendLine(
                    StatePrettyPrinterVocabulary.buildListSameString(
                        key = key,
                        size = oldSize,
                        indentSize = context.indentSize,
                    ),
    ...
    private fun <T> StringBuilder.appendEntryChanges(
        ...
        var current = 0
        appendLine(openingMarker.prependIndent(size = context.indentSize))
        while (oldIterable?.hasNext() == true || newIterable?.hasNext() == true) {
            ...
            val formattedOldValue = formatValue(oldValue)
            appendLine(
                if (oldValue != newValue) {
                    StatePrettyPrinterVocabulary.formatIterableItemWithChange(
                        ...
                        indentSize = nestedContext.indentSize,
                    )
                } else {
        ...
        appendLine(closingMarker.prependIndent(size = context.indentSize))
    }
}

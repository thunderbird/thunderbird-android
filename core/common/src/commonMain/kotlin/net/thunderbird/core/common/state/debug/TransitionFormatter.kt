...
internal class TransitionFormatter<TState : Any, TEvent : Any>(
    ...
    fun buildTransitionRecord(
        ...
        if (elapsed.isNotEmpty()) append(elapsed.prependIndent(size = 2))
        appendLine()

        val eventProps = record.event.toPropertyMap()
        if (eventProps.isNotEmpty()) {
            appendLine("${record.event::class.simpleName} payload:".prependIndent(size = 2))
            for ((key, value) in eventProps) {
                appendLine("$key: ${valueFormatter.format(value)}".prependIndent(size = 4))
            }
        }

        appendLine("${new::class.simpleName} updated data:".prependIndent(size = 2))
        append(
            differ.buildDiff(
                old = prev,
                new = new,
                context = DiffContext(indentSize = 4, isStateClassChanged = isStateClassChanged),
            ),
        )
    }

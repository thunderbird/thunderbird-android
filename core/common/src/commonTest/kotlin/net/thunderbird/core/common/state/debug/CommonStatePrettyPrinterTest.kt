...
class CommonStatePrettyPrinterTest {
    ...
    fun `prettyPrint - full dump should contain all records in order`() {
        ...
        historyStack.addLast(
            createRecord(
                ...
                timestamp = Instant.fromEpochMilliseconds(600),
            ),
        )
        ...
        assertThat(stateTransitions).all {
            ...
            transform { transitions -> transitions.map { it.trim().lines().first() } }
                .containsExactly(
                    ...
                    "Loading ${StatePrettyPrinterVocabulary.STATE_NO_CHANGE_MARKER} Loading  event=UpdateProgress  +200ms",
                    "Loading ${StatePrettyPrinterVocabulary.STATE_CHANGE_MARKER} Done  event=Finish  +300ms",
                )
        }
    }
    ...
    private fun createTestSubject(
        ...
        fullDumpCooldown: Duration = 2.seconds,
    ) = CommonStatePrettyPrinter<TestState, TestEvent>(
        logger = logger,
        logTag = logTag,
        clock = clock,

...
open class AndroidFileLogSink(
    ...
    private var job: Job = coroutineScope.launch(start = CoroutineStart.LAZY) { writeToLogFile() }
    ...
    override fun log(event: LogEvent) {
        try {
            accumulatedLogs.add("${convertLongToTime(event.timestamp)} priority = ${event.level}, ${event.message}")
    ...
    private fun writeToLogFile() {
        val result = runCatching {
            ...
            try {
                val content = accumulatedLogs.joinToString("\n", postfix = "\n")
                val buffer = Buffer()
